        ...
        Action::PeerMessageReadSuccess(content) => {
            ...
            match &content.message.message() {
                ...
                PeerMessage::GetProtocolBranch(msg) => {
                    slog::warn!(store.state().log, "Received GetProtocolBranch message: {msg:?}, ignoring it");
                }
                PeerMessage::GetPredecessorHeader(msg) => {
                    slog::warn!(store.state().log, "Received GetPredecessorHeader message: {msg:?}, ignoring it");
                }
                _ => {}
            }
